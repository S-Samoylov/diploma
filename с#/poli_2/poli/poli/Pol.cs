...
namespace poli
{
    class Pol
    {
        ...
        /*
        private static Dictionary<string, int> pValues;

        public static void prepareDict2(int l)
        {
            pValues = new Dictionary<string, int>();
            LIMIT = l;
        }*/

        public static List<int> pValues;
        ...
        private static List<int> walkTree(int currentConNumber, int currentPolValue, int depth)
        {
            ...
            List<int> pValuesFut = new List<int>();
            if (depth == 0)
            {
                for (int i = currentConNumber + 1; i < Conjunction.conValues.Length; i++)
                {
                    if (Conjunction.conValues[i] == inputValue)
                    {
                        List<int> result = new List<int>() { currentConNumber };
                        return result;
                    }
                    pValues.Add(Conjunction.conValues[i]);
                }
                return walkTree(1, pValues[0], 1);
            }
            else
            {
                for (int i = currentConNumber + 1; i < Conjunction.conValues.Length; i++)
                {
                    ...
                    if (vecSum == inputValue)
                    {
                        List<int> l = new List<int>() { i };
                        return l;
                    }
                    pValuesFut.Add(Conjunction.conValues[i]);
                }
                //walkTree(currentConNumber + 1, pValues.IndexOf, depth);
            }
