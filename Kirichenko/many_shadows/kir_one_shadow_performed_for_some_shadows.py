...
if (__name__ == "__main__"):
    ...
    for coverage in coverages:
        pol = []
        res = kir_with_min(f, cover = coverage, param = 0)
        pol.append(len(res))
        pol.append(res)
		
        after_min = logic_minimize(res)
        pol.append(len(after_min))
        pol.append(after_min)
        ...
        while (True):
            ...
            if (s == polo):
                ...
                polo = s
        performed = logic_minimize(polo)

        pol.append(len(performed))
        pol.append(performed)
        if (value_by_poly(performed) != [int(x) for x in f]):
            df = open("log.txt","a");
            df.writelines(f)
            df.writelines(str(poli_output(performed)))
            df.close()
        if (len(performed) < min_len):
            min_len = len(performed)
            min_pol = copy.deepcopy(pol)
    print('Kirichenko')
    print(min_pol[0])
    print(poli_output(min_pol[1]))
    print('Min_Kirichenko')
    print(min_pol[2])
    print(poli_output(min_pol[3]))
    print('Performed_min_Kirichenko')
    print(min_pol[4])
    print(poli_output(min_pol[5]))